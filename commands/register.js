exports.run = (client, message, args, config) => {
	...
	let nationURL = `https://www.nationstates.net/cgi-bin/api.cgi?nation=${nation}&q=wa+region`;
	...
	request({
		...
		parseString(body, function(error, result) {
			...
			if (data.REGION[0].split(" ").join("_").toLowerCase() !== config.region) {
				...
				return message.channel.send(`You have been assigned the role **${config.roles.foreign}**. If you are here as a diplomat, please send a message to a government member.`);
				...
				return message.channel.send(`You have been assigned the role **${config.roles.citizen}**. Please endorse the WA Delegate if you haven't already.`);
			} else {
